...
Future<void> _salvarAvaliacaoNoFirestore(Map<String, dynamic> avaliacao, {String? docId}) async {
  ...
  try {
    await _salvarOuAtualizarAvaliacao(avaliacoesRef, avaliacao, docId, user.uid);
          ...
          onSave: (avaliacao) async {
            ...
            if (index != null) {
              ...
              final querySnapshot = await avaliacoesRef
                  .where('comentario', isEqualTo: avaliacoes[index]["comentario"])
...
ListView.builder(
  ...
  itemBuilder: (context, index) {
    final avaliacao = avaliacoes[index];
    return Dismissible(
      key: Key(avaliacao["comentario"]), // Chave única para cada avaliação
      direction: DismissDirection.endToStart, // Deslize da direita para a esquerda
      ...
      onDismissed: (direction) async {
        ...
        try {
          final querySnapshot = await avaliacoesRef
              .where('comentario', isEqualTo: avaliacao["comentario"])
              .where('nomeUsuario', isEqualTo: avaliacao["local"])
              .get();
