...
class LocalUserService {
    ...
    try {
      ...
      if (avaliacoesSnapshot.docs.isNotEmpty) {
        ...
        await _db
            .collection('locais_usuario')
            .doc(localId)
            .update({'mediaEstrelas': mediaEstrelas});
      } else {
        await _db
            .collection('locais_usuario')
            .doc(localId)
            .update({'mediaEstrelas': 0});
      }
    } catch (e) {
      _logFirestoreError(e);
    }
  ...
  Future<void> addAvaliacao(
      String localId, Map<String, dynamic> avaliacao) async {
    try {
      await _db
          .collection('locais_usuario')
          .doc(localId)
  ...
  Future<void> updateAvaliacao(String localId, String avaliacaoId,
      Map<String, dynamic> avaliacao) async {
    try {
      await _db
          .collection('locais_usuario')
          .doc(localId)
  ...
  Future<List<LocalUserModel>> fetchLocaisUsuario() async {
    ...
    try {
      ...
      return snapshot.docs
          .map((doc) => LocalUserModel.fromJson(doc.data()))
          .toList();
    } catch (e) {
      _logFirestoreError(e);
      return [];
  ...
  Future<double> getMediaEstrelas(String localId) async {
    try {
      final snapshot =
          await _db.collection('locais_usuario').doc(localId).get();
      if (snapshot.exists) {
